...
defmodule BankAccountTest do
  ...
  # @tag :pending
  test "initial balance is 0", %{account: {:ok, account}} do
    assert BankAccount.balance(account) == 0
  end

  # @tag :pending
  test "incrementing and checking balance", %{account: {:ok, account}} do
    assert BankAccount.balance(account) == 0
  ...
  # @tag :pending
  test "amount is added to balance", %{account: {:ok, account}} do
    assert BankAccount.balance(account) == 0
    BankAccount.update(account, 10)
    BankAccount.update(account, 10)
  ...
  # @tag :pending
  test "incrementing balance from another process then checking it from test process", %{account: {:ok, account}} do
    assert BankAccount.balance(account) == 0
    this = self()
    spawn(fn ->
